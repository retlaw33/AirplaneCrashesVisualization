function GenerateWorldMap() 
{
    ...
    var svg = d3v3.select('#worldmapPlaceHolder').append('svg')
    ...
    d3v3.json('data/worldmapdata.json', function(error, mockdata) {
    ...
    function draw(data) {
        ...
        //     }
        // }
        d3v3.json('data/world.json', function(error, world) {
            if (error) return console.error(error);
...
var worldmapFired = 0;
window.addEventListener("scroll", makeWorldMapWhenInView);

function makeWorldMapWhenInView()
{
	var isElementInView = Utils.isElementInView($('#worldmapHeader'), false);

	if (isElementInView && worldmapFired == 0) 
	{
		GenerateWorldMap();
		worldmapFired = 1;
	} 
